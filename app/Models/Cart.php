...
class Cart extends Model
{
    ...
    public function add()
    {
        ...
        if (!$product) {
            throw new ModelNotFoundException('product not found by ID ' . $this->product);
        }
    ...
    public function increase()
    {
        ...
        if (FacadesCart::instance('main')->merge(auth()->user()->id) == true) {
            ...
            if ($checkExist->isEmpty()) {
                throw new ModelNotFoundException('product already found in cart by ID ' . $this->product);
            } else {
                $cartSession = FacadesCart::instance('main');
                foreach ($checkExist as $check) {
                    $row = $check->rowId;
    ...
    public function decrease()
    {
        ...
        if (FacadesCart::instance('main')->merge(auth()->user()->id) == true) {
            ...
            if ($checkExist->isEmpty()) {
                throw new ModelNotFoundException('product not found in cart by ID ' . $this->product);
            } else {
                $cartSession = FacadesCart::instance('main');
                foreach ($checkExist as $check) {
                    $row = $check->rowId;
